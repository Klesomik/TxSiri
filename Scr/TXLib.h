...
#define _TX_VERSION           _TX_V_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 105, 2014-08-09 16:37:26 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR            _TX_A_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 105, 2014-08-09 16:37:26 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
#define _TX_VER               _TX_v_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 105, 2014-08-09 16:37:26 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
//! @{ @cond INTERNAL
    ...
    #define  strcpy_s               strcpy      // MSVC prior to 8 (2005) versions and GCC (unless having bounds-checker
    #define  strncpy_s              strncpy     //   library extension do NOT have secure variants of these functions,
    #define  strncat_s              strncat     //   so use insecure ones.
    ...
    #if !( defined (_GCC_VER) && (_GCC_VER >= 471) && defined (__x86_64__) )  // At least GCC 4.7.1 x64 already has it
...
void _txCodeBegin() _TX_NOINLINE;
void _txCodeBegin() {};
...
//!
//! @see     txPrintCallStack(), TX_ERROR(), TX_DEBUG_ERROR()
//!
//! @usage @code
...
#define txPrintCallStack()  _txPrintCallStack (__FILE__, __LINE__, __TX_FUNCTION__, true);
...
void             _txPrintCallStack           (const char file[] = "(\?\?)", int line = 0, const char func[] = "(\?\?)",
...
FARPROC _txDllImport (const char dllFileName[], const char funcName[], bool required /*= true*/)
    ...
    if (arch)
        {
        strncpy_s (dllName, dllFileName, arch - dllFileName);
        strncat_s (dllName, arch+1, sizeof (dllName) - 1 - strlen (dllName));

        strncpy_s (dllArch, dllFileName, arch - dllFileName);
        strncat_s (dllArch, sizeof (void*) == 8? "64" : "32", 3);
        strncat_s (dllArch, arch+1, sizeof (dllArch) - 1 - strlen (dllArch));
        }
    else if (dllFileName)
        strncat_s (dllName, dllFileName, sizeof (dllName) - 1);
...
#if defined (SYMOPT_CASE_INSENSITIVE)

#undef SYMOPT_CASE_INSENSITIVE
#undef SYMOPT_UNDNAME
#undef SYMOPT_DEFERRED_LOADS
#undef SYMOPT_NO_CPP
#undef SYMOPT_LOAD_LINES
#undef SYMOPT_OMAP_FIND_NEAREST
#undef SYMOPT_LOAD_ANYTHING
#undef SYMOPT_IGNORE_CVREC
#undef SYMOPT_NO_UNQUALIFIED_LOADS
#undef SYMOPT_FAIL_CRITICAL_ERRORS
#undef SYMOPT_EXACT_SYMBOLS
#undef SYMOPT_ALLOW_ABSOLUTE_SYMBOLS
#undef SYMOPT_IGNORE_NT_SYMPATH
#undef SYMOPT_INCLUDE_32BIT_MODULES
#undef SYMOPT_PUBLICS_ONLY
#undef SYMOPT_NO_PUBLICS
#undef SYMOPT_AUTO_PUBLICS
#undef SYMOPT_NO_IMAGE_SEARCH
#undef SYMOPT_SECURE
#undef SYMOPT_NO_PROMPTS
#undef SYMOPT_ALLOW_ZERO_ADDRESS
#undef SYMOPT_DISABLE_SYMSRV_AUTODETECT
#undef SYMOPT_FAVOR_COMPRESSED
#undef SYMOPT_FLAT_DIRECTORY
#undef SYMOPT_IGNORE_IMAGEDIR
#undef SYMOPT_OVERWRITE
#undef SYMOPT_DEBUG

#endif

const unsigned    SYMOPT_CASE_INSENSITIVE          = 0x00000001,
...
#undef       EXCEPTION_POSSIBLE_DEADLOCK
#endif
...
const unsigned    EXCEPTION_BUFFER_OVERFLOW               =   0xC0000409,
                  EXCEPTION_POSSIBLE_DEADLOCK             =   0xC0000194,
                  STATUS_FLOAT_MULTIPLE_FAULTS            =   0xC00002B4,
                  STATUS_ASSERTION_FAILURE                =   0xC0000420,

                  EXCEPTION_CPP_MSC                       =   0xE06D7363,  // '?msc'
                  EXCEPTION_CPP_MSC_EH_MAGIC_NUMBER1      =   0x19930520,  // '?msc' version magic, see ehdata.h
                  EXCEPTION_CPP_MSC_EH_MAGIC_NUMBER2      =   0x19930521,  // '?msc' version magic
                  EXCEPTION_CPP_MSC_EH_MAGIC_NUMBER3      =   0x19930522,  // '?msc' version magic
                  EXCEPTION_CPP_MSC_EH_PURE_MAGIC_NUMBER1 =   0x01994000,  // '?msc' version magic

                  EXCEPTION_CPP_GCC                       =   0x20474343,  // ' GCC'
                  EXCEPTION_CPP_GCC_UNWIND                =   0x21474343,  // '!GCC'
                  EXCEPTION_CPP_GCC_FORCED                =   0x22474343,  // '"GCC'

                  EXCEPTION_CLR_FAILURE                   =   0xE0434f4D,  // 'аCOM'

                  EXCEPTION_CPP_BORLAND_BUILDER           =   0x0EEDFAE6,  // Should never occur here
                  EXCEPTION_CPP_BORLAND_DELPHI            =   0x0EEDFADE,  // Should never occur here

                  STATUS_WX86_BREAKPOINT                  =   0x4000001F,
                  EXCEPTION_OUTPUT_DEBUG_STRING           =   0x40010006,  // OutputDebugString() call
                  EXCEPTION_THREAD_NAME                   =   0x406D1388;
//}
//-----------------------------------------------------------------------------------------------------------------
...
char                    _txTraceSE [_TX_BIGBUFSIZE] = "";     // SEH dump data area
...
$       _set_invalid_parameter_handler (_txOnInvalidParam);
        ...
        #if defined (_MSC_VER) && (_MSC_VER == VS2003)
...
$       ::std::set_constraint_handler_s (_txOnSecurityErrorAnsi);
        ...
        #if defined (_MSC_VER) && (_MSC_VER >= 1 /*!!!*/) || defined (_GCC_VER) && (_GCC_VER >= 530)
...
void _txOnUnexpected()
    ...
    _txError (NULL, 0, NULL, 0x0D /* LightMagenta */,
...
long WINAPI _txVectoredExceptionHandler (EXCEPTION_POINTERS* exc)
    {
    return _txOnExceptionSEH (exc, __func__);
    }
...
long WINAPI _txUnhandledExceptionFilter (EXCEPTION_POINTERS* exc)
    {
    long res = _txOnExceptionSEH (exc, __func__);
...
$       strncpy_s           (_txTraceSE, _txCaptureStackBackTrace (0), sizeof (_txTraceSE) - 1);
...
$       _txCurLoc = loc;
        ...
        _txError (NULL, 0, NULL, (func[3] == 'V'? 0x03 /* Cyan */ : 0x0B /* LightCyan */), "\a"
                  "%s"
...
$   HMODULE module = NULL;
$   GetModuleHandleEx (GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, (const char*) &addr, &module);

$   static char sModule [MAX_PATH] = "";
$   GetModuleFileName (module, sModule, sizeof (sModule));

$   char* ext = strrchr (sModule, '.');
$   if (ext) _strlwr_s (ext, sizeof (sModule) - 1 - (ext - sModule));

$   PRINT_ (" в модуле %s. Спасибо %s(), что сообщил. Люблю его <3" _ sModule _ func);
...
$           break;
        ...
        case 0:  // Not called within SEH chain
            {
            // From: [1] http://github.com/gcc-mirror/gcc/blob/master/libstdc%2B%2B-v3/libsupc%2B%2B/eh_type.cc
            //       [2] http://github.com/gcc-mirror/gcc/blob/master/libstdc%2B%2B-v3/libsupc%2B%2B/vterminate.cc

$           ABI::__cxa_exception* cxa_exception = ((ABI::__cxa_eh_globals*) abi::__cxa_get_globals()) -> caughtExceptions;
...
const char* _txCaptureStackBackTrace (int framesToSkip /*= 0*/, bool readSource /*= true*/)
    ...
    int frames = Win32::RtlCaptureStackBackTrace (framesToSkip + 1, SIZEARR (capture), capture, NULL);

    memset (trace, 0, sizeof (trace));
    char* s = trace;

    #define PRINT_(msg)  s += _tx_snprintf_s (s, sizeof (trace) - 1 - (s-trace), msg)
    ...
    for (int i = 0; i < frames; i++)
        ...
        if (!(_txCodeBegin <= addr && addr <= _txCodeEnd) &&
            !(sym && sym->Name && strstr (sym->Name , "::TX::")))
        ...
        if (module)                   PRINT_ ("%s!" _        module);
                                      PRINT_ ("%s" _        (sym && *sym->Name)? sym->Name : "(\?\?)");
//      if (offset && offset)         PRINT_ ("+0x%llx" _    offset);
...
unsigned _txSymGetFromAddr (void* addr /*= NULL*/, Win32::SYMBOL_INFO** symbol /*= NULL*/,
    ...
    if (!process)
        ...
        DWORD options = SYMOPT_UNDNAME | SYMOPT_LOAD_LINES | SYMOPT_LOAD_ANYTHING | SYMOPT_INCLUDE_32BIT_MODULES |
            ...
            #define RES_(sym)       (strncat_s (res, #sym, sizeof (res) - 1), 1)
    ...
    if (module)
        ...
        DWORD64 mod = 0;
                    mod = CALL_(MinGW::SymGetModuleBase64, (process, (uintptr_t) addr)); if (mod) RES_(m);
        if (!mod) { mod = CALL_(Win32::SymGetModuleBase64, (process, (uintptr_t) addr)); if (mod) RES_(M); }
        if (!mod) DBG_ ("SymGetModuleBase64() failed");

        GetModuleFileName ((HMODULE) mod, sMod, MAX_PATH);
    ...
    if (source)
        ...
        if (source) RES_($); else DBG_ ("_txReadText (\"%s\") failed", line64.FileName);
        }
...
ptrdiff_t _txReadText (char buf[], ptrdiff_t size, const char file[],
    ...
    #define SZ_  ( size - (s - buf) )

    while (!feof (f) && SZ_ > 0)
        {
        if (n > linEnd || SZ_ < 0) break;
    ...
    if (n <= linEnd && SZ_ <= 0 && size >= (ptrdiff_t) sizeof (ellipsis) - 1)
        {
        s -= sizeof (ellipsis) - 1;
        s += _tx_snprintf_s (s, SZ_, ellipsis);
        }
...
const char* _txReportError (const char file[], int line, const char func[], unsigned color, const char msg[], va_list args)
    ...
    const char*    stkTrace = _txCaptureStackBackTrace (1, true);
    ...
    if (crterr) PRINT_ (", errno: %d (%s)" _      crterr _ (strerror_s (str, crterr), str));
    if (doserr) PRINT_ (", _doserrno: %lu (%s)" _ doserr _ (strerror_s (str, doserr), str));
...
$   txOutputDebugPrintf ("\r" "%s - %s", _TX_VERSION, what);
    ...
    txSetConsoleAttr (isFatal? 0x0D : 0x0C);  // LightMagenta / LightRed
...
int txOutputDebugPrintf (const char format[], ...)
    ...
    struct __ { static void compressSpaces (char* dst, const char* src)
        {
        for (char d = ' '; d; src++)
...
//!!! if (compr)  __::compressSpaces (str, str);
...
ptrdiff_t _tx_vsnprintf_s (char stream[], ptrdiff_t size, const char format[], va_list arg)
    {
    ptrdiff_t ret = _vsnprintf_s (stream, size _TX_TRUNCATE, format, arg);
/*!!!
    if (ret < 0 && size >= 3)
        {
        const char ellipsis[] = "...";
        ...
        #if defined (_MSC_VER) && (_MSC_VER >= 1400) || defined (__STDC_LIB_EXT1__)
        strncpy_s (stream + size - sizeof (ellipsis) - 1, sizeof (ellipsis) - 1, ellipsis, sizeof (ellipsis) - 1);
        #else
        strncpy   (stream + size - sizeof (ellipsis) - 1,                        ellipsis, sizeof (ellipsis) - 1);
        #endif
        }
*/     
...
const char* txGetModuleFileName (bool fileNameOnly /*= true*/)
    ...
    if (!*name)
        {
        GetModuleFileName (NULL, name, sizeof (name) - 1) || strcpy_s (name, "");
