...
//  if (strlen (__DOT__) + strlen (From) + strlen (To) + strlen (Type) + OtherSymbols >= ComandSize) dtError ("Names too long!");
    if (                   strlen (From) + strlen (To) + strlen (Type) + OtherSymbols >= ComandSize) dtError ("Names too long!");

// "iconv -f cp866 -t utf-8
//  sprintf (Comand, "%s -T%s %s -o %s", __DOT__, Type, From, To);

    sprintf (Comand, "type %s | dot\\iconv -f cp1251 -t utf-8 | dot\\dot -T%s -o %s", From, Type, To);
